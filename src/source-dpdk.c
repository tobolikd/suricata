...
void ReceiveDPDKSetRings(DPDKThreadVars *ptv, DPDKIfaceConfig *iconf, uint16_t queue_id)
{
    ...
    memcpy(ptv->rings.idxes_offlds_suri_requested, iconf->idxes_offlds_suri_requested[queue_id], sizeof(ptv->rings.idxes_offlds_suri_requested));
    memset(iconf->idxes_offlds_suri_requested[queue_id], 0, sizeof(iconf->idxes_offlds_suri_requested[queue_id]));
    ptv->rings.cnt_offlds_pf_requested = iconf->cnt_offlds_suri_support;
    iconf->cnt_offlds_suri_support = 0;
    memcpy(ptv->rings.idxes_offlds_pf_requested, iconf->idxes_offlds_suri_support, sizeof(ptv->rings.idxes_offlds_pf_requested));
