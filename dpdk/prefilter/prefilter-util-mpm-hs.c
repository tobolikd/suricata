...
// TODO* remove #ifdef BUILD_HYPERSCAN
...
pref_hs_matches_t prefilter_hs_matches = { .signature_ids = NULL, .sinature_size = 0 };
 ...
 * \retval hash A 32 bit unsigned hash.
 */
static inline SCHSPattern *SCHSInitHashLookup(
        SCHSCtx *ctx, uint8_t *pat, uint16_t patlen, uint16_t offset, uint16_t depth, uint32_t pid)
{
        ...
        uint16_t depth, uint32_t pid, SigIntId sid, uint8_t flags)
{
    ...
    SCHSPattern *p = SCHSInitHashLookup(ctx, pat, patlen, offset, depth, pid);
    if (p == NULL) {
...
static PatternDatabase *PatternDatabaseAlloc(uint32_t pattern_cnt)
{
    ...
    pd->parray = (SCHSPattern **)SCMalloc(pd->pattern_cnt * sizeof(SCHSPattern *));
    if (pd->parray == NULL) {
        SCFree(pd);
        return NULL;
    }
...
int SCHSPreparePatterns(MpmCtx *mpm_ctx)
{
    ...
    SCHSCompileData *compile_data = NULL;
    PatternDatabase *pattern_db = NULL;

    compile_data = SCHSAllocCompileData(mpm_ctx->pattern_cnt);
    if (compile_data == NULL) {
        goto error;
    }

    pattern_db = PatternDatabaseAlloc(mpm_ctx->pattern_cnt);
    if (pattern_db == NULL) {
        goto error;
    }
    ...
    PatternDatabase *pd_cached = HashTableLookup(g_db_table, pattern_db, 1);
    ...
    if (pd_cached != NULL) {
        ...
        SCMutexUnlock(&g_db_table_mutex);
        PatternDatabaseFree(pattern_db);
        SCHSFreeCompileData(compile_data);
        return 0;
    }

    BUG_ON(ctx->pattern_db != NULL); /* already built? */

    for (uint32_t i = 0; i < pattern_db->pattern_cnt; i++) {
        const SCHSPattern *p = pattern_db->parray[i];

        compile_data->ids[i] = i;
        compile_data->flags[i] = HS_FLAG_SINGLEMATCH;
        compile_data->flags[i] = HS_FLAG_PREFILTER;
        if (p->flags & MPM_PATTERN_FLAG_NOCASE) {
            compile_data->flags[i] |= HS_FLAG_CASELESS;
        }

        compile_data->expressions[i] = HSRenderPattern(p->original_pat, p->len);

        if (p->flags & (MPM_PATTERN_FLAG_OFFSET | MPM_PATTERN_FLAG_DEPTH)) {
            compile_data->ext[i] = SCMalloc(sizeof(hs_expr_ext_t));
            if (compile_data->ext[i] == NULL) {
                SCMutexUnlock(&g_db_table_mutex);
                goto error;
            }
            memset(compile_data->ext[i], 0, sizeof(hs_expr_ext_t));

            if (p->flags & MPM_PATTERN_FLAG_OFFSET) {
                compile_data->ext[i]->flags |= HS_EXT_FLAG_MIN_OFFSET;
                compile_data->ext[i]->min_offset = p->offset + p->len;
            }
            if (p->flags & MPM_PATTERN_FLAG_DEPTH) {
                compile_data->ext[i]->flags |= HS_EXT_FLAG_MAX_OFFSET;
                compile_data->ext[i]->max_offset = p->offset + p->depth;
            }
        }
    }
    ...
    err = hs_compile_ext_multi((const char *const *)compile_data->expressions, compile_data->flags,
            compile_data->ids, (const hs_expr_ext_t *const *)compile_data->ext,
            compile_data->pattern_cnt, HS_MODE_BLOCK, NULL, &pattern_db->hs_db, &compile_err);
    ...
    ctx->pattern_db = pattern_db;

    SCMutexLock(&g_scratch_proto_mutex);
    err = hs_alloc_scratch(pattern_db->hs_db, &g_scratch_proto);
    ...
    err = hs_database_size(pattern_db->hs_db, &ctx->hs_db_size);
    if (err != HS_SUCCESS) {
    ...
    pattern_db->ref_cnt = 1;
    int r = HashTableAdd(g_db_table, pattern_db, 1);
    SCMutexUnlock(&g_db_table_mutex);
    if (r < 0)
        goto error;
        ...
        unsigned int flags, void *ctx)
{
    ...
    // TODO* turn this into own callback PrefilterAddSids(pmq, pat->sids, pat->sids_size);
...
// TODO* remove #endif /* BUILD_HYPERSCAN */
